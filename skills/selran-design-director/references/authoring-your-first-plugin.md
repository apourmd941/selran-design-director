...
- Open a GitHub issue against the [Selran repo](https://github.com/apourmd941/Selron-design-director-skill) tagged `plugin-author-question` if anything is unclear. Plugin contract feedback in the first 90 days post-launch will shape `plugin_contract_version: "1.1"` (additive changes only — see the schema-freeze policy).
